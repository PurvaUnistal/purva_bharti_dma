...
class ApiIntegration {
  static Future<dynamic> getTestData({var endPoint, BuildContext context}) async {
  ...
  static Future<dynamic> getData({var urlEndPoint, var setApiData, BuildContext context}) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    var token = prefs.getString(GlobalConstants.token);
    try {
  ...
  Future<ChangePasswordModel> changePasswordApi(ChangePasswordResponse changePasswordResponse, BuildContext context) async {
  ...
  Future<SaveCustomerRegistrationModel> saveCustRegApi(BuildContext context,
    ...
    try {
      if(await isInternetConnected() == true){
        ...
        if (response.statusCode == 200) {
          ...
          var error =  jsonDecode(responseData)['errors'] ?? "";
          CustomToast.showErrorSnackBar(error.toString().replaceAll("[{", "").toString().replaceAll("}]", "").toString(),context);
          return null;
        } else if(response.statusCode == 500){
          CustomToast.showErrorSnackBar("Server Error", context);
          return null;
        }
        else {
